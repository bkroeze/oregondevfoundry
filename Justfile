...
image := env_var_or_default("IMAGE", "oregon-dev-foundry:local")
...
push: build
    ...
    if [[ "{{image}}" != */* ]]; then
      ...
      echo "Example: IMAGE=ghcr.io/owner/oregon-dev-foundry:latest just push" >&2
      exit 2
    fi
    docker push "{{image}}"
...
run port=default_port: build
    docker run --rm --name "{{container_name}}" -e PORT="{{port}}" -p "{{port}}:{{port}}" "{{image}}"

# Run the container detached; e.g. `just up 3000`.
up port=default_port: build
    docker rm -f "{{container_name}}" >/dev/null 2>&1 || true
    docker run -d --name "{{container_name}}" -e PORT="{{port}}" -p "{{port}}:{{port}}" "{{image}}"
